...
import { getClient, handleRealTimeMessages, processPdf } from './config/audioai.js';
    ...
    try {
        ...
        await aiClient.send({
          ...
          response: {
            ...
            instructions: `Say "Hello" to start the conversation.`,
          }
        });
        ws.on("message", async (message) => {
            try {
              ...
              switch (data.type) {
                case "process_pdf": {
                  await processPdf(aiClient, ws, data.pdfText);
                  break;
                }
                case "session_update": {
                  await aiClient.send({
                ...
                case "process_pdf_file": {
                  const pdfTextChunks = await extractPdfText(data.fileData);          
                  const pdfText = pdfTextChunks.join(" ");
                  messages.push(`This is text for analysis: \n ${pdfText}`);
                  await processPdf(aiClient, ws, pdfText);
                  break;
                }
